...
In multitasking applications (when using [[tasks][lib/tasks.frt]]) SLIH routines
always execute with the main user area resources selected.
...
This is a rewrite ("clean-up") of lib/multitask.frt (trunk@1650) with minor
improvements such as the addition of task names and simpler task
invocation. Note that [[SLIH][soft interrupt routines]] use the main task user area
and should not be counted when allocating memory to each task.

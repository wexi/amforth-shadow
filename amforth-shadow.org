...
- ffff :: [ASM] This is an abort call that occurs when forgetting to
     resolve a forward reference. In other words, when executing erased
     Flash "code". α (aka \1) records the IP where ffff is called.
...
For references calls or values should be marked with ellipsis:
#+BEGIN_EXAMPLE
… my-last-word … ['] my-last-word
#+END_EXAMPLE

The #forward directive lists the unresolved references. #resolve, based on
an #update-words view, would introduce the necessary Flash code changes.

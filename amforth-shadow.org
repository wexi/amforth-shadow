...
In multitasking applications, that is, when using lib/multitasking.frt,
SLIH routines always execute with the first user area resources selected
("ram_user1").
...
Since the worlist order is kept on the EEPROM it is good practice to reduce

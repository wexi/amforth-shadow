...
EE_DP, EE_HERE and EE_EDP now have RAM_DP, RAM_HERE and RAM_EDP copies.  
IMPORTANT: Execute EESY ( -- ) to sync these EE pointers with their RAM copies. 
This is done automatically by tools/amforth-shell.py before leaving the program.
...
+ [[eesy][EESY]] ( -- ) :: [ASM] Sync memory allocation pointers.

...
any other AVR pass his core/devices/at90can128/device.asm changes to your
device.asm of interest.

The commercial product that includes AmForth-Shadow at the time of this
writing is considered "Release Candidate" quality. Hence the elevation of
env-forthversion.asm to V1.0.
...
* TODO Flash programming, faster and safer 

The current method of Flash programming (!i) is both inefficient and
unsafe. Inefficient, since one can program 128× faster through pages;
Unsafe, since we erase the Flash 128× more than necessary and the Flash is
only guaranteed 10,000 erase/write cycles.

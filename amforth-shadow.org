...
by an experienced embedded systems programmer, yet a novice[fn:1] in
Forth. It is hoped that the ideas that are presented here would be adopted
by [[http://amforth.sourceforge.net/][AmForth]] headquarters (HQ). The following summarizes the Shadow changes
to HQ's code.
...
* <<SLIH>> Second Level Interrupt Handlers (SLIH), aka Soft Interrupts

One of AmForth features is the ability to write Interrupt Service Routines
(ISR) in Forth. However, still in HQ's trunk@1650 those ISRs should run
with the hardware interrupt system disabled. This means that one has to
keep ISRs short and defer long processing to the "main".

The following changes to the kernel introduce an eight level soft
interrupts queue, allowing the Forth code Second Level Interrupt Handlers
(SLIH) to be interruptible and thus be as slow to execute as necessary. The
new first level interrupt handler (FLIH) that "feeds" the SLIH queue is
suitable for type I interrupt sources only, those acknowledged by execution
of their interrupt vectors. However, in addition, ...
...
+ int+ ( -- ) "soft interrupts on" :: Enables soft interrupts if the number
     of int+ calls is greater or equal than the number of preceding int-
     calls. Thus, an int- int+ pair can enclose any piece of code that
     needs to run with the soft interrupt system disabled irrespective of
     the initial state of the soft interrupts, on or off.

+ int- ( -- ) "soft interrupts off" :: Disables soft interrupts. If this is
     the first SLIH routine word, the routine will not be interrupted.
...
enhancements such as the addition of task names. NOTE: Since SLIH routines
operate as (resume execution of) the main task, there is no need to
intersperse PAUSE calls in a compute only task as any high frequency SLIH
clock service would do so for you anyway.
...
+ task-init ( ITC TASK -- ) :: initialize task in memory: ITC is some
     endless code with pauses. TASK is the task name. For example:
#+BEGIN_EXAMPLE
$20 $40 0 task: TASK
...
- +!@ ( w1 addr -- w2 ) :: [ASM] Adds w1 to addressed word and returns sum.

- ++c ( addr -- b ) :: [ASM] increments addressed byte, returns new value.

- c++ ( addr -- b ) :: [ASM] increments addressed byte, returns original value.
...
- fdrop  ( X -- 0 ) :: [ASM] Replace top of stack with 0 (false).

- tdrop  ( X -- -1 ) :: [ASM] Replace top of stack with -1 (true).
...
- wild :: [ASM] Returns the word-list of the last word created. This is
          used by [[tasks][lib/tasks.frt]] to easily access the task name. As WLSCOPE
          can place created words on different word-lists CREATE using WILD
          can compile different code.

- main :: [ASM] Returns the main task user area start.

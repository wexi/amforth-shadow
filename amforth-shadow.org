...
independent fork of the [[http://amforth.sourceforge.net/][AmForth]] project since its trunk@1687 decision to
change its license from GPLv2 to GPLv3. Earlier to that decision the
AmForth project was fondly called "HQ" as there was still some hope that it
would adopt some of AmForth-Shadow innovations.

AmForth-Shadow is maintained by an experienced embedded systems programmer
who considers himself yet a novice[fn:1] in Forth. 
...
The following changes to the kernel introduce an INTQUE deep, 8 level by
default, interrupt queue, allowing Forth code Second Level Interrupt
Handlers (SLIH) to be launched that can be as slow to execute as
necessary. The new first level interrupt handler (FLIH) that "feeds" the
SLIH queue is suitable for type I interrupt sources only, those
acknowledged by execution of their interrupt vectors. However, in addition:

...
words/greek.asm uses the AVR VM otherwise unused [fn:2] registers ah:al
(r7:r6) "α", bh:bl (r9:r8) "β" and ch:cl (r11:r10) "γ" to create three
Forth Greek alphabet named locals. Here is an example where "(3)" is
responsible for initializing the locals from the stack. Use "(2)" or "(1)"
if your word is using two locals ("α" "β") or just one ("α").

#+CAPTION: Functionality Test
#+BEGIN_EXAMPLE
: test1 (3)  α . β . γ .  ;
: test2 (3)  α . β . γ . α 1+ β 1+ γ 1+ test1 α . β . γ .  ;
> 1 2 3 test2 
1 2 3 2 3 4 1 2 3  ok
#+END_EXAMPLE

#+CAPTION: Typical use
#+BEGIN_EXAMPLE
: sort  ( α=addr β=length γ=order -- ) (3) your-favorite-sort-algorithm ;
#+END_EXAMPLE

Note: AmForth-Shadow supports UTF-8 dictionary names.

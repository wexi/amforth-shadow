...
Since trunk@1548 HQ attempts to pack as much as possible asm code into the core. What asm code goes into the core is determined solely by the MCU architecture NRWW block size -- 8k, 4k or 2k. The
shadow complained that this new method causes an overflow when BOOFA is needed on the NRWW block as well and suggested an alternative code packing method. Until HQ adopts an alternative method, when
BOOFA is needed, the least intrusive solution that the shadow has found is to step down the MCU core ".inc list". See the first few lines in core/dict/core_8k.inc, etc.

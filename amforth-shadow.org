...
[[conceal][--conceal]] and [[log][--log]]. Further development is needed in this connection,
see the [[NOTE][the note below]].
...
+ --conceal :: <<conceal>> Substitute earlier captured word names with
               serial numbers.\\ <<NOTE>> NOTE: A limitation that has
               yet to be addressed is how to detect for concealment
               names which were altered through the WLSCOPE
               mechanism. Alternatively, and perhaps better, is the
               removal of these words from the captured word list which
               ~--create~ had generated.

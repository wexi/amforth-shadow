...
CAN interrupts, if the application CANQUE asm switch is "1", are safely routed to the queue by CANGIE.ENIT auto disable. The Forth code should acknowledge the specific CAN interrupt cause before
resetting ENIT.
...
The shadow is proud to have contributed the Word List Scope idea and implementation. Still remains:

** DONE words/does.asm bug-fix

DOES> was erronesously using get-current to access the last created code which, through WLSCOPE mechanism, can be different.

* DONE words/icompare.asm bug-fix

Signed comparison prevented us until now from enjoying UTF-8 strings. Not any more.

: : Σ 0 swap 0  do  +  loop  ;  ( X1 X2 .. Xn n -- sigma )

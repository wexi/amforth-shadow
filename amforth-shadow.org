...
* USART tx/rx isr with DTR/RTS/CTS support

Define the following macros in your application main file ("template.asm") as necessary and as applicable to your project.

#+BEGIN_EXAMPLE
#define RXR_SIZE 16		;receive buffer size override
#define TXR_SIZE 32		;transmit buffer size override
...
.macro	IS_RTS_ON
  sbic	PINE, 6
.endmacro
.macro	IS_RTS_OFF	;hold off serial output
  sbis	PINE, 6
.endmacro
...
.macro	DTR_IN
  cbi	DDRE, 7
.endmacro
.macro	IS_DTR_ON	;skip if host is up
  sbis	PINE, 7
.endmacro
.macro	IS_DTR_OFF	;skip if host is down
  sbic	PINE, 7
.endmacro
#+END_EXAMPLE

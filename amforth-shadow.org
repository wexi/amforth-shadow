...
If I just had the time I would have replaced AmForth dictionary names with 16bit hash values. The dictionary names when development is through can simply be "forgotten".  In additional to the obvious
code space saving reverse engineering of code via "see" would become significantly harder.

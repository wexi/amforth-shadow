...
** words/does.asm bug-fix

DOES> was erronesously using get-current to access the last created code which, through WLSCOPE mechanism, can be different.

* TODO UTF-8

Unicode UTF-8 strings are currently accepted by ." and s". Currently though UTF-8 dictionary words cannot be found (therefore used). The shadow would like to enter code like:
